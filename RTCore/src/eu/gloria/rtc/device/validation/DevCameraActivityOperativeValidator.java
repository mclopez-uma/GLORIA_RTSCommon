...
/**
 * Validates if a Camera device is ready to receive a request (Operative) taking into account the activity state.
 * @author jcabello
 *
 */
public class DevCameraActivityOperativeValidator extends DevActivityStateValidator {
	
	/**
	 * List of right activity states
	 */
	protected List<ActivityStateCamera> rightStates;
	
	/**
	 * Constructor.
	 * @param device Device to check.
	 */
	public DevCameraActivityOperativeValidator(Device device){
		
		super(device);
		
		rightStates.add(ActivityStateCamera.READY);
	 ...
	 * @return Device
	 */
	private DeviceCamera getDevice(){
		return (DeviceCamera) this.device;
	}
